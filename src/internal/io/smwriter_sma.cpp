...
void SMwriter_sma::write_vertex(const float* v_pos_f)
{
  ...
  fprintf(file, "v %g %g %g\012",v_pos_f[0],v_pos_f[1],v_pos_f[2]);
  v_count++;
}
...
void SMwriter_sma::write_header()
{
  ...
  if (bb_min_f) fprintf(file, "# bb_min %g %g %g\012",bb_min_f[0],bb_min_f[1],bb_min_f[2]);
  if (bb_max_f) fprintf(file, "# bb_max %g %g %g\012",bb_max_f[0],bb_max_f[1],bb_max_f[2]);
}

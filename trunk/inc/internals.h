...
/* C glue needed for spread-write stuff */
#include "csp_glue.h"
...
/* Labels that signify the finalizer mode. NONE indicates that the finalier  */
/*   is not used                                                             */
enum finalizermode { NO_FINALIZER, COARSE_MODE, COUNTER_MODE, CHUNKING_MODE };

/* Labels that signify the finalizer mode. NONE indicates that the finalier  */
/*   is not used                                                             */
enum regionattributeindex { REGION_ID };

enum filetypes { NODE_FILE, SMA_FILE, SMB_FILE, SPA_FILE, SPB_FILE,
		 POLY_FILE, ELE_FILE, AREA_FILE  };
...
struct mesh {
  ...
  /* Fine triangulation related variables */
  int vertextricountindex;     /* Index to find num of triangles touching    */
                               /* a vertex.                                  */
  int waitingforindex;         /* Index to find which coarse region the fine */
                               /* triangle is waiting for.                   */
  int outfinenodemarker;            /* Fine vertex marker                    */
  int outfineelementmarker;         /* Fine element marker.                  */
...
struct behavior {
  ...
  /* Flags indicating whether finalization is in place, and if so, which stage */
  enum finalizermode finalize;
  int clarkson;
  unsigned long long timestamp;                            /* Global timestamp */
  
  SPwriter *point_writer;
  SMwriter *header_writer;
  SMreader *mesh_reader;
  SPreader *point_reader;
  FILE *mesh_file;
  FILE *point_file;
  FILE *header_file;
  int spa, spb, sma, smb;
  ...
  char outspafilename[FILENAMESIZE];
  char outspbfilename[FILENAMESIZE];
  char outsmafilename[FILENAMESIZE];
  char outsmbfilename[FILENAMESIZE];
...
//-- for reconstruction fine mesh ---
#define vertextricount(vx) ((int *) (vx))[m->vertextricountindex]

#define setvertextricount(vx, value)		\
  ((int *) (vx))[m->vertextricountindex] = value

#define incvertextricount(vx)                   \
  ((int *) (vx))[m->vertextricountindex]++

#define decvertextricount(vx)                   \
  ((int *) (vx))[m->vertextricountindex]--
...
/* triextra.c functions */
typedef void (*dfs_visitor)(struct mesh *m, struct behavior *b, vertex p, region *rgn, unsigned int depth);
typedef void (*triangle_visitor)(struct mesh *m, struct behavior *b, struct otri tri, int number);

#ifdef ANSI_DECLARATORS
void write_header(struct mesh *m, struct behavior *b);
void write_points_header(struct mesh *m, struct behavior *b, int points);
void bowyer_watson(struct mesh *m, struct behavior *b, vertex p, dfs_visitor previsit, dfs_visitor postvisit);
...
void process_finalized_region(struct mesh *fm, struct mesh *cm, struct behavior *b, int id);
void construct_finalization(struct mesh *m, struct behavior *b, FILE **polyfile);
void read_triangulation(struct mesh *m, struct behavior *b);
...
void set_triangle_id(struct mesh *m, struct behavior *b, struct otri tri, int id);
...
#else /* not ANSI_DECLARATORS */
void write_header();
void write_points_header();
void bowyer_watson();
void counter_visit();
void chunking_pre_visit();
void chunking_post_visit();
void process_finalized_region();
void construct_finalization();
int initialize_point_reader();
int initialize_mesh_reader();
void set_triangle_id();
void finetriangulate();

...
void syntax()
{
#ifdef CDT_ONLY
#ifdef REDUCED
  printf("triangle [-pAcjevngfkBPNEIOXzo_lQVh] input_file\n");
#else /* not REDUCED */
  printf("triangle [-pAcjevngfkBPNEIOXzo_iFlCQVh] input_file\n");
#endif /* not REDUCED */
#else /* not CDT_ONLY */
#ifdef REDUCED
  printf("triangle [-prq__a__uAcDjevngfkBPNEIOXzo_YS__lQVh] input_file\n");
#else /* not REDUCED */
  printf("triangle [-prq__a__uAcDjevngfkBPNEIOXzo_YS__iFlsCQVh] input_file\n");
#endif /* not REDUCED */
#endif /* not CDT_ONLY */
  ...
  */
  ...
  printf("    -g  Generates an .off file for Geomview.\n");
	printf("    -f  Generates finalization information for -k\n");
  printf("    -k  Generates a fine triangulation based on the output of -f\n");
...
void info()
{
  ...
  printf("        Triangle is buggy.\n");
	...
	printf(
	 "    -f  Finalizer: Preprocesses the input point set (.node) and outputs\n"
	 );
	printf(
   "        finalization information (.sma and .spa) for the Fine Triangulator.\n"
	 );
	printf(

   "        The output includes a coarse triangulation (.sma in Ascii or .smb in\n"
	 );
	printf(
   "        binary) and a fine point set with finalization tags (.spa in ascii\n"
	 );
	printf(
   "        or .spb in binary). This flag is only used for triangulating with\n"
	 );
	printf(
   "        streaming (-k flag). Please see -k flag for more details.\n"
	 );
	printf(
   "    -k  Fine Triangulator: Generates a fine triangulation with streaming.\n"
	 );
	printf(
   "        Takes the output of the Finalizer (see -f) as input, and generates\n"
	 );
	printf(
   "        a triangulation accordingly. This uses less memory footprint since\n"
	 );
	printf(
   "        finalized parts of the data structure are output to the disk as soon\n"
	 );
	printf(
   "        as it reads a finalization tag. The final output will be exactly the\n"
	 );
	printf(
   "        same as triangulating with other algorithms.\n"
	 );
  printf(
...
#define STARTINDEX 1
  ...
  b->finalize = NO_FINALIZER; /* 0 */
  b->poly = b->refine = b->quality = 0;
  b->spa = b->spb = b->sma = b->smb = 0;
  b->clarkson = 0;
  ...
  for (i = STARTINDEX; i < argc; i++) {
	...
	b->innodefilename[strlen(b->innodefilename) - 5] = '\0';
      ...
      if (!strcmp(&b->innodefilename[strlen(b->innodefilename) - 4], ".spa")) {
	b->innodefilename[strlen(b->innodefilename) - 4] = '\0';
	b->spa = 1;
	strcpy(b->outspafilename, b->innodefilename);
	strcat(b->outspafilename, ".spa");
	printf("<<you want to open file %s>>\n", b->outspafilename);
      }
      if (!strcmp(&b->innodefilename[strlen(b->innodefilename) - 4], ".spb")) {
	b->innodefilename[strlen(b->innodefilename) - 4] = '\0';
	b->spb = 1;
	strcpy(b->outspbfilename, b->innodefilename);
	strcat(b->outspbfilename, ".spb");
	printf("<<you want to open file %s>>\n", b->outspbfilename);
      }
      if (!strcmp(&b->innodefilename[strlen(b->innodefilename) - 4], ".sma")) {
	b->innodefilename[strlen(b->innodefilename) - 4] = '\0';
	b->sma = 1;
	strcpy(b->outsmafilename, b->innodefilename);
	strcat(b->outsmafilename, ".sma");
	printf("<<you want to open file %s>>\n", b->outsmafilename);
       }
      if (!strcmp(&b->innodefilename[strlen(b->innodefilename) - 4], ".smb")) {
	b->innodefilename[strlen(b->innodefilename) - 4] = '\0';
	b->smb = 1;
	strcpy(b->outsmbfilename, b->innodefilename);
	strcat(b->outsmbfilename, ".smb");
	printf("<<you want to open file %s>>\n", b->outsmbfilename);
       }
  ...
  /* use segments for reconstructing coarse triangle if clarkson flag is on  */
  b->usesegments = b->poly || b->refine || b->quality || b->convex || b->clarkson;
  ...
  if (!b->refine && !b->clarkson && !b->poly) {
    b->vararea = 0;
  }
  /* Be careful not to add an extra attribute to each element unless the */
  /*   input supports it (PSLG in, but not refining a preexisting mesh   */
  /*   or fine triangulating). */
  if (b->refine || b->clarkson || !b->poly) {
    b->regionattrib = 0;
  }
  ...
  if (b->noiterationnum) {
    ...
    strcpy(b->neighborfilename, b->innodefilename);
    strcpy(b->offfilename, b->innodefilename);
    strcat(b->outspafilename, ".1.spa");
    strcat(b->outspbfilename, ".1.spb");
    strcat(b->outsmafilename, ".1.sma");
    strcat(b->outsmbfilename, ".1.smb");
  ...
  // add file extension 
  if (b->spa) {
    strcat(b->innodefilename, ".spa");
  } else if (b->spb) {
    strcat(b->innodefilename, ".spb");
  } else if (b->sma) {
    strcat(b->innodefilename, ".sma");
  } else if (b->smb) {
    strcat(b->innodefilename, ".smb");
  } else {
...

{
  ...
  /* The index within each vertex at which triangle counter is found  */
  /*   It is used for streaming. Ensure the vertex is removed when no */
  /*   triangle is touching it */
  m->vertextricountindex = m->vertexmarkindex+1;

  vertexsize = (m->vertextricountindex + 2) * sizeof(int);
  if (b->poly) {
...
/*                                                                           */
/* The operations Fast_Two_Sum(), Fa_Two_Diff(), Two_Sum(), Two_Diff(),    */
...

{
  ...
  while (1) {
    ...
    if (fapex && (fapex[0] == searchpoint[0]) && (fapex[1] == searchpoint[1])) {
      lprevself(*searchtri);
      return ONVERTEX;
    }
    
    /* If not ghost triangle */
    if (forg && fapex && fdest) {
    ...
    /*If ghost triangle (Mario added) */
    else {

      struct otri adjacent;
      int location;

      location = ininfiniteregion(m, b, searchtri, forg, fdest, fapex, searchpoint, &adjacent);
...

{
  ...
  /*
    ...
    Do not perform check if torg/tdest is ghost vertex
  */
  
  if (torg && (torg[0] == searchpoint[0]) && (torg[1] == searchpoint[1])) {
    return ONVERTEX;
  }
  
  if (tdest && (tdest[0] == searchpoint[0]) && (tdest[1] == searchpoint[1])) {
    lnextself(*searchtri);
    return ONVERTEX;
  }
  
  if (torg && tdest) {
    /* Orient `searchtri' to fit the preconditions of calling preciselocateg(). */
...

{
  ...
  if ((intersect == ONEDGE) || (intersect == OUTSIDE)) {
    ...
    // jack
    //TODO: double check newvertex, is it 2?
    incvertextricount(botvertex);
    setvertextricount(newvertex, 2);
    ...
    if (mirrorflag) {
      ...
      //jack
      incvertextricount(topvertex);
      setvertextricount(newvertex, 4);
    ...
    incvertextricount(leftvertex);
    incvertextricount(rightvertex);
    incvertextricount(botvertex);
    setvertextricount(newvertex, 3); // 3 triangles touching it
  ...
  while (1) {
    ...
    if (doflip) {
      ...
      // TODO:  make sure we hanlde hole too
	  ...
	  /* farvertex newvertex's counter++
	   * leftvertex rightvertex's counter-- */
	  // jack
	  //TODO: double check here
	  incvertextricount(farvertex);
	  incvertextricount(newvertex);
	  decvertextricount(leftvertex);
	  decvertextricount(rightvertex);
...

{
  ...
  /* If not in finalizer mode, remove ghost triangles */
  /* TODO: it would not work with ghost triangles removed, so i comment this out first */
...

{
  ...
  if (b->finalize) {
    printf("Delaunay is called by Finalizer\n");
    m->eextras = 1;
  } else {
    m->eextras = 0;
  }
...
/*                  .poly) file.  Used when the -r or -k switch is used.     */
/*                                                                           */
...

{
  ...
  m->eextras = attribs;
#else /* not TRILIBRARY */
  /* If streaming mesh file */
  if (b->sma || b->smb) {
    /* Read the triangles from an .ele file. */
    printf("Opening %s.\n", elefilename);
    /* Read number of triangles from streaming mesh file */
    m->inelements = smreader_nfaces(b->mesh_reader);
    /* Set number of triangle corners to 3 */
    incorners = 3;
    /* Set number of attributes of triangle to 1 (triangle id) */
    m->eextras = 1;
  } 
  /* If regular ele file */
  else {
  ...
  //  m->elemattribindex = 9;
  /*
  if( m->elemattribindex < 6 ) {
    printf("Warnging! elemattribindex is only %d\n", m->elemattribindex );
    triexit(1);
  }
  */
  ...
  if( b->sma || b->smb ) {
    // jack added
    /* Allocate an array of finalization regions. Regions have useful info */
    /* such as region id and count for proccessing finalization later.     */
    m->finalization_regions = (region *) malloc(sizeof(region) * m->inelements);
  }
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    printf("Reconstructing %d-th triangle\n", elementnumber);
#ifdef TRILIBRARY
    ...
    for (j = 0; j < 3; j++) {
      ...
      if ((corner[j] < b->firstnumber) ||
        ...
        printf("Error:  Triangle %ld has an invalid vertex index %d.\n", elementnumber, corner[j]);
        triexit(1);
      }
    }
#else /* not TRILIBRARY */
    ...
    if (b->sma || b->smb) {
      ...
      /* Read next element from streaming mesh, must be triangle */
      if (!smreader_read_element(b->mesh_reader)) {
	fprintf(stderr, "Error reading face");
	triexit(1);
      }
      
      /* Obtain vertex ids */      
      vertices = smreader_t_idx(b->mesh_reader);
      
      /* For every triangle corner */
      for (j = 0; j < 3; j++) {
	
	corner[j] = vertices[j] + 1;
    ...
    /* If not reading from streaming mesh file */
    if (!b->sma && !b->smb) {
    ...
    /* If clarkson reconstruction, assign number to triangle */
    if (b->clarkson) {
      setelemattribute(triangleloop, REGION_ID, elementnumber);
    }
    ...
    if (b->vararea) {
     ...
     /* Set the triangle's vertices, while ensuring that if clarkson reconstruction infinite triangle created correctly */
    triangleloop.orient = 0;
    if(b->clarkson && !corner[0]) {
      setorg(triangleloop, NULL);
    } else {
      setorg(triangleloop, getvertex(m, b, corner[0]));
    }
    
    if(b->clarkson && !corner[1]) {
      setdest(triangleloop, NULL);
    } else {
      setdest(triangleloop, getvertex(m, b, corner[1]));
    }
    
    if(b->clarkson && !corner[2]) {
      setapex(triangleloop, NULL);
    } else {
    ...
    printf("Done linking triangle\n");

    if( b->sma || b->smb ) { // if reconstrucing coarse mesh

      // set triangle id
      //      set_triangle_id(m, b, triangleloop, elementnumber);
  
      // map the coarse finalization region to the orient triangle
      m->finalization_regions[elementnumber].tri = triangleloop;
      m->finalization_regions[elementnumber].tri.orient = 0;

      //      printf("new triangle orient = %d\n" ,m->finalization_regions[elementnumber].tri.orient );

      if( m->finalization_regions[elementnumber].tri.tri == (triangle*) NULL ) {
	printf("Error: finalize region %ld is NULL!", elementnumber);
      }
      //      m->finalization_regions[elementnumber].tri.orient = 0;
      // initialized to not finalized! turns this off (to 0) when we finalize it
      m->finalization_regions[elementnumber].count = 1;
      // all other fields are not useful and ignored
      m->finalization_regions[elementnumber].timestamp = -1; // whatever
      m->finalization_regions[elementnumber].visited = -1; // whatever
      m->finalization_regions[elementnumber].head = NULL;
      m->finalization_regions[elementnumber].tailptr = NULL;
    }

    triangleloop.tri = triangletraverse(m);

    // set to 0, otherwise it would be 3, which doesn't make sense
    //    triangleloop.orient = 0;
  ...
  vertexindex = 0;
#else /* not TRILIBRARY */
  /* If not streaming mesh file, close elefile */
  if (!b->sma && !b->smb) { 
    fclose(elefile);
  }
  if (b->vararea) {
...

{
  ...
  if (regions > 0) {
    ...
    if (b->regionattrib && !b->refine && !b->clarkson) {
...
//jack note:
// If readnodes() is reading .spa or .spb file (i.e: type==SPA_FILE or SPB_FILE)
//    *cm is the coarse mesh reconstructed from .sma or .smb file previously
// Otherwise, *cm is NULL

#ifndef TRILIBRARY

#ifdef ANSI_DECLARATORS
void readnodes(struct mesh *m, struct behavior *b, char *nodefilename,
               char *polyfilename, FILE **polyfile, enum filetypes type, struct mesh *cm)
#else /* not ANSI_DECLARATORS */
  void readnodes(m, b, nodefilename, polyfilename, polyfile, type, cm)
     struct mesh *m;
     struct behavior *b;
     char *nodefilename;
     char *polyfilename;
...

{
  ...
  /* whether or not we want to construct the mesh (DT).      */
  /* true iff b->finalize is in NO_FINALIZER or COARSE_MODE  */
  /* notice this includes reading sma, smb, spa, spb files   */
  int construct_dt;

  /* total # of fine points. ie: # of vertices we read in */
  int num_fine_points;

  /* total Number of coarse vertices                             */
  /* this = num_fine_points unless b->finalize is in COARSE_MODE */
  int num_coarse_vertices;

  /* Number of selected coarse vertices */
  int num_selected_coarse_vertices;

  /* Number of remaining vertices to be read */
  int num_rem_vertices;
  ...
  if( b->finalize == NO_FINALIZER || b->finalize == COARSE_MODE ) {
    construct_dt = 1; // construct a DT for m is needed
  } else {
    construct_dt = 0; // m is already a constructed mesh
  }
  ...
  if (b->poly) {
    ...
    *polyfile = fopen(polyfilename, "r");
    ...
    /* number of vertices */
    num_fine_points = (int) strtol(stringptr, &stringptr, 0);
  ...
  if (m->readnodefile) {
    ...
    if( (type==SMA_FILE && !b->sma)
	|| (type==SMB_FILE && !b->smb)
	|| (type==SPA_FILE && !b->spa)
	|| (type==SPB_FILE && !b->spb) ) {
      printf("Required file is not passed to triangle");
      triexit(1);
    }
    ...
    /* Node markers 0 unless specified */
    nodemarkers = 0;
    
    /* If reading from streaming point file */
    if ( type==SPA_FILE || type==SPB_FILE ) {
      /* Initialize point reader */
      num_fine_points = initialize_point_reader(m, b, infile);
      printf("num fine points: %d\n", num_fine_points);
    }
    /* If reading from streaming mesh file */
    else if ( type==SMA_FILE || type==SMB_FILE ) {
      /* Initialize mesh reader */
      num_fine_points = initialize_mesh_reader(m, b, infile);
    }
    /* If reading from a regular .node file */
    else {
  ...
  /* Initialize number of vertices remaining to be read */
  num_rem_vertices = num_fine_points;
  
  if(b->finalize == COARSE_MODE && num_fine_points > 9) {
    /* 
       Set number of invertices to number of coarse vertices to be selected,
       ensuring at least 3 coarse vertices will be selected
    */
    num_coarse_vertices = sqrt(num_fine_points);
  } else {
    num_coarse_vertices = num_fine_points;
  }

  // set m->vertices and construct the vertext pool only if we are constructing DT (the mesh)
  if(construct_dt) {
    m->invertices = num_coarse_vertices;
    printf("Construct DT=> # of vertices = %d \n", m->invertices);
    initializevertexpool(m, b);
  }
  
  printf("Beginning of reading vertices \n");
  printf("Mode=%d, %d fine points, %d points to process, m->invertices=%d \n",
	 b->finalize, num_fine_points, num_coarse_vertices, m->invertices );
  
  /* If in chunking mode, prepare points header */
  if (b->finalize == CHUNKING_MODE) {
    write_points_header(m, b, num_fine_points); 
  }

  if ( b->clarkson && (type==SPA_FILE || type==SPB_FILE) ) {
    initializetrisubpools(m, b);
    boundingbox(m, b);
  }
  
  /*---------------------- Read the vertices. -----------------------------------*/
  for (/* Initialize number of coarse vertices selected */
       num_selected_coarse_vertices = 0;
       /* While non-zero probability of selection of vertex */
       num_selected_coarse_vertices < num_coarse_vertices;
       /* Decrement number of vertices to be read */
       num_rem_vertices--) {
    ...
    if ( type==SPA_FILE || type==SPB_FILE ) {
      ...
      /* Read next point coordinates (but finalize if we encounter finalization cells) */
      do {
	/* Read event */
	pevent = spreader_read_event(b->point_reader);
	if( pevent == SP_FINALIZED_CELL ) {	  
	  // jack: watch out here
	  if (b->clarkson) {
	    process_finalized_region(m, cm, b, spreader_final_idx(b->point_reader));
	  }
	} else if (!pevent) {
	  printf("Error:  EOF while more fine points are expected.");
	  triexit(1);
	}
      }
      /* Repeat until done with finalization records */
      while (pevent != SP_POINT);
    }
    /* If reading from streaming mesh file */
    else if ( type==SMA_FILE || type==SMB_FILE ) {
      /* Read vertex coordinates */
      mevent = smreader_read_element(b->mesh_reader);
      if( !mevent ) {
	printf("Error:  EOF while more coarse points are expected.");
	triexit(1);
      }
    }
    /* If regular .node file simply read line */
    else {
      stringptr = readline(inputline, infile, infilename);
    }
    
    /* If coarse triangulation of finalizer */
    if (b->finalize == COARSE_MODE) {
      /* If vertex not selected */
      if (randomnation(num_rem_vertices) + num_selected_coarse_vertices >= num_coarse_vertices) {
	/* Proceed to reading next vertex */
	continue;
      }
    }
    ...
    if (num_selected_coarse_vertices == 0) {

      // jack TODO: notice the 'firstnumber' when we are doing sm file is diff from the one for sp file

      /* If coarse triangulation of finalizer or .sp type file*/
      if (b->finalize==COARSE_MODE || type==SPA_FILE || type==SPB_FILE) {
	/* Mark first selected vertex as vertex 1 */
	b->firstnumber = firstnode = 1;
      } else if ( type==SMA_FILE || type==SMB_FILE ) {
	b->firstnumber = firstnode = 0;
      }
      else {
    ...
    /* If reading from streaming point file */
    if ( type==SPA_FILE || type==SPB_FILE ) {
      /* If encountered error or EOF prematurely, exit */
      if (pevent != SP_POINT) {
	printf("Error reading streaming point file\n");
	triexit(1);
      }
      /* Else if point is good */
      else {
    ...
    /* If reading from streaming mesh file */
      ...
      /* If encountered error or EOF/triangle prematurely, exit */
      if (mevent != SM_VERTEX) {
	printf("Error reading streaming mesh file\n");
	triexit(1);
      }
      /* Else if vertex is good */
      else {
	
	/* Obtain vertex from reader */
	float *point = smreader_v_pos_f(b->mesh_reader);
    ...
    /* If reading from regular .node file */
    else {
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
	printf("Error:  Vertex %d has no x coordinate.\n", b->firstnumber + num_coarse_vertices);
	triexit(1);
      }
	...
	stringptr = findfield(stringptr);
	// only put attributes if we are constructing DT
	if( construct_dt ) {
	  if (*stringptr == '\0') {
	    vertexloop[j] = 0.0;
	  } else {
    ...
    /* Note that nodemarkers should be set to false if reading from .spa or .spb file */
    if (nodemarkers) {
    ...
    /*
      Update bounding box if:
      1) Constructing triangulation
      2) Counting step of finalizer and input is not .spa or .spb
     */
    if ((construct_dt && !b->clarkson)||
	(!b->spa &&
	 !b->spb &&
	 !b->sma &&
	 !b->spb &&
    ...
    /* now we have all the information about this fine vertex */
    switch( b->finalize ) {
    case COUNTER_MODE:
      //printf("coarse...\n");
      bowyer_watson(m, b, vertexloop, counter_visit, NULL);
      pooldealloc(&m->vertices, vertexloop);
      break;
    case CHUNKING_MODE:
      //printf("chunking...\n");
      bowyer_watson(m, b, vertexloop, chunking_pre_visit, chunking_post_visit);
      pooldealloc(&m->vertices, vertexloop);
      break;
    default: break; /* do nothing */
    }
    
    /* Increment count of coarse vertices */
    num_selected_coarse_vertices++;

  } /*------------- end of reading vertices loop ------------------*/
  
  if (b->clarkson && ( type==SPA_FILE || type==SPB_FILE )) {    
    // finish up reading the finalization tags
    /* Read event */
    SPevent pevent;
    while( (pevent = spreader_read_event(b->point_reader)) ) {
      if( pevent == SP_FINALIZED_CELL ) {
	process_finalized_region(m, cm, b, spreader_final_idx(b->point_reader));
      } else {
	printf("Error: Unexpected fine point encountered. There are more fine points than expected. ");
	triexit(1);
      }
    }
    removebox(m, b);
  ...
  if (m->readnodefile) {
    ...
    /* If not streaming mesh file (if it is, it's closed at a later time as it may be used to read triangles) */
    else if ( type!=SMA_FILE && type!=SMB_FILE ) { 
      fclose(infile);
    }
  }
...
//jack: need to transfer vertextricount for streamming
...
#ifndef CDT_ONLY
  if ((b->regionattrib || b->vararea) && !b->refine && !b->clarkson) {
...

{
  ...
  /* Quality statistics unsupported under finalization mode for now */
  if (b->verbose && !b->finalize) {
...

{
  ...
  if(b.clarkson && (b.spa || b.spb) && !(b.sma || b.smb) ) {
    printf("In order to triangulate, coarse mesh file (.sma or .smb) is required!\n");
    triexit(1);
  }
#endif /* not TRILIBRARY */
  m.steinerleft = b.steiner;

  if(b.clarkson && (b.spa || b.spb)) {
    triangleinit(&fm);
    // jack: double check here
    fm.steinerleft = b.steiner;
  }
...
#else /* not TRILIBRARY */
  readnodes(&m, &b, 
	    (b.sma ? b.outsmafilename : (b.smb ? b.outsmbfilename : b.innodefilename)),
	    b.inpolyfilename, &polyfile,
	    // pass file type
	    (b.sma ? SMA_FILE : (b.smb ? SMB_FILE : (b.spb ? SPB_FILE : (b.spa ? SPA_FILE : NODE_FILE)))),
	    (struct mesh*) NULL);
#endif /* not TRILIBRARY */
...
#else /* not CDT_ONLY */
  if (b.refine || b.clarkson ) { // if reconstruct or clarkson
    ...
    m.hullsize = reconstruct(&m, &b,
			     (b.sma ? b.outsmafilename : (b.smb ? b.outsmbfilename : b.inelefilename)),
			     b.areafilename, b.inpolyfilename, polyfile);
    
    if(b.clarkson && (b.spa || b.spb)) { // if clarkson is selected and fine points are given
      // fine triangulate
      finetriangulate(&fm, &b,
		      (b.spa ? b.outspafilename : b.outspbfilename),
		      &m, argc, argv);
    }
#endif /* not TRILIBRARY */
  } else {
  ...
  if (!b.quiet) {
    gettimeofday(&tv3, &tz);
    if (b.usesegments && !b.refine && !b.clarkson) {
  ...
  if ( b.nonodewritten || (b.noiterationnum && m.readnodefile) || b.finalize ) {
    if (!b.quiet ) {
    ...
    printf("NOT Writing coarse triangulation nodes\n");
    printf("Fine triangulation nodes and elemtns have been written.\n");

     if( b.clarkson && (b.spa || b.spb) ) {
       // do nothing because fine triagnulation nodes have been written.
       printf("output node file name = %s\n", b.outnodefilename);

//       printf("Writing fine triangulation nodes\n");

// #ifdef TRILIBRARY
//     writenodes(&fm, &b, &out->pointlist, &out->pointattributelist,
//                &out->pointmarkerlist);
// #else /* not TRILIBRARY */
//     strcpy(b.outnodefilename, "finemesh.node");
//     writenodes(&fm, &b, b.outnodefilename, argc, argv);
// #endif /* TRILIBRARY */
  ...
  if (b.noelewritten) {
    ...
    if(b.finalize) {
      construct_finalization(&m, &b, &polyfile);

    }  else if (b.clarkson) {
    /* Allocate region map */

      strcpy(b.outsmbfilename, "blabla.smb");
      strcpy(b.outsmafilename, "blabla.sma");
      m.regions=m.inelements;
      write_header(&m, &b);
      printf("Not writing coarse triangulation elements\n");

      if( b.spa || b.spb ) {

	// do nothing because fine triangulation elements have been written
// 	printf("output element file name = %s\n", b.outelefilename);

//  	printf("Writing fine triangulation elements (triangles)\n");
//  	strcpy(b.outelefilename, "finemesh.ele");
//  	writeelements(&fm, &b, b.outelefilename, argc, argv);
      }
    } else {

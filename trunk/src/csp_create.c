...
int main(int argc, char *argv[])
{
  ...
  if (strstr(file_name_header_out, ".sma"))
  {
    ...
    smwriter_open(smwriter, file_header_out);
  }
  else if (strstr(file_name_header_out, ".smb"))
  {
  ...
  if (file_name_out || ospa || ospb)
  {
    ...
    if ((file_name_out && strstr(file_name_out, ".spa")) || ospa)
    {
      ...
      spwriter_open(spwriter, file_out);
    }
  ...
  smwriter_set_nverts(smwriter, clarkson2d_mesh_nverts);
  smwriter_set_nfaces(smwriter, clarkson2d_mesh_nfaces);
  smwriter_set_boundingbox(smwriter, bb_min_f, bb_max_f);

  for (i = 0; i < clarkson2d_mesh_nverts; i++)
  {
    smwriter_write_vertex(smwriter, clarkson2d_mesh_verts[i]);
  }

  for (i = 0; i < clarkson2d_mesh_nfaces; i++)
  {
    smwriter_write_triangle(smwriter, clarkson2d_mesh_faces[i]);
  }

  smwriter_close(smwriter);
  fclose(file_header_out);
  delete_smwriter(smwriter);

  // *then* we write the finalized point stream

  spwriter_set_npoints(spwriter, data_npoints);
  spwriter_set_boundingbox_f(spwriter, bb_min_f, bb_max_f);
  spwriter_set_datatype(spwriter, SP_FLOAT);
  spwriter_set_finalizemethod(spwriter, SP_CLARKSON_2D);

  spwriter_write_header(spwriter);

  // write all the points

  for (i = 0; i < data_npoints; i++)
  {
    spwriter_write_point_f(spwriter, data_points[i]);
  }
  ...
  for (i = 0; i < clarkson2d_mesh_nfaces; i++)
  {
    spwriter_write_finalize_cell(spwriter, i);
  }
  ...
  spwriter_close(spwriter);
  if (file_out != stdout) fclose(file_out);
  delete_spwriter(spwriter);

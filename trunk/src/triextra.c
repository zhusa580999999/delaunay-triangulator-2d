...
void outputTriangle(struct mesh* m, struct behavior *b, struct otri t, FILE *outfile) {
  t.orient= 0;
  vertex v[3];
  getvertices(t, v);
    ...
    if( isInSection ) {
      ...
      if ( isFinalized( cm, neighbors[other_region] ) ) { // is it also finalized?
				...
				if( !isFineTriInfinite( fm, triangleloop )) { // output if this is finalized triangle
					...
					// outputTriangle( fm, b, triangleloop, outfile );
				}
...
#endif /* not ANSI_DECLARATORS */
{
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    if( !isFineTriInfinite( m, triangleloop) ) {
			// printf("there\n");
      outputTriangle( m, b, triangleloop, outelefile );
    }
    triangleloop.tri = triangletraverse(m);
  }
